...
func increase_by_sec(delta_seconds: float) -> void: # (float palang yung data)
	delta_time += delta_seconds
	if delta_time < 1: return # return lng ng whole numbb
...
func diff_without_days(other_time: DateTime) -> int:
	...
	var diff_seconds = seconds - other_time.minutes + diff_minutes * 60
